...
export const metadata: Metadata = {
  ...
  description:
    'Buy a digital Tailgate discount card and start saving at local businesses.',
  openGraph: {
    title: 'Get a Tailgate Digital Card',
    description:
...
async function OrganizationBuyPage({ params }: PageProps) {
  const { slug } = await params;

  const client = getSupabaseServerClient();

  const { data, error } = await client.rpc('get_organization_buy_page', {
    p_slug: slug,
  });
...
export default withI18n(OrganizationBuyPage);
