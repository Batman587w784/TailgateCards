...
interface DistributorShareLinkCardProps {
  shareSlug: string | null;
  className?: string;
}

export function DistributorShareLinkCard({
  shareSlug,
  className,
}: DistributorShareLinkCardProps) {
    ...
    <Card className={cn('gap-3 md:gap-5', className)}>
      ...
      <CardContent className="space-y-3">
        <button
          ...
          data-test="distributor-share-copy"
        >
          <span className="flex-1 truncate" data-test="distributor-share-link">
            {displayUrl}
          </span>
          {copied ? (
        ...
        <Button
          ...
          onClick={handleShare}
          data-test="distributor-share-button"
        >
          Share Sales Link
        </Button>
        <p className="text-muted-foreground text-xs">
          Share this link with your friends to sell digital cards.
        </p>
      </CardContent>
    </Card>
  );
